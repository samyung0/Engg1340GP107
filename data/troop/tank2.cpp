...
void Tank2::giveDamage(double disruption, double airSupremacy, double mul1, double mul2, double mul3, double mul4, Damage &damage)
{
...
{
  hp -= damage;
  pivotalStrength = std::pow(1.2 - std::exp(-1.5 * (hp / Tank2::baseHp) + std::log(0.2) + 1.5), 1 - (hp / Tank2::baseHp));
}
